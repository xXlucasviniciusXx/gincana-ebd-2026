...
- **Ranking (`escape_ranking` view):** finished teams first → fewest `rejected_photos` → smallest **effective time** = `(finished_at − started_at) + penalty_seconds`. Penalties accumulate in `escape_team_state.penalty_seconds`: **+`wrong_penalty_seconds` (default 30 min) per wrong answer**, **+`hint_penalty_seconds` (default 15 min) per hint** (both configurable in admin Configurações). Quiz/enigma steps gate progression (must be correct to advance); photo steps auto-accept (clue on upload) and are admin-reviewed — rejecting sets `photo_review='rejected'` (a ranking demerit). Image-puzzle types (answer in the filename, hidden word, QR) are just `riddle` steps with an `image_url`.
